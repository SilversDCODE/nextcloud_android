...
class SyncedFolderUtilsTest : com.owncloud.gshare.AbstractIT() {
    @Test
    ...
    fun assertMediaFolderNullSafe() {
        val folder: com.owncloud.gshare.datamodel.MediaFolder? = null
        Assert.assertFalse(SyncedFolderUtils.isQualifyingMediaFolder(folder))
    }

    @Test
    fun assertMediaFolderCustomQualified() {
        val folder = com.owncloud.gshare.datamodel.MediaFolder()
        folder.type = com.owncloud.gshare.datamodel.MediaFolderType.CUSTOM
        Assert.assertTrue(SyncedFolderUtils.isQualifyingMediaFolder(folder))
    }

    @Test
    fun assertMediaFolderVideoUnqualified() {
        val folder = com.owncloud.gshare.datamodel.MediaFolder().apply {
            absolutePath = getDummyFile(THUMBDATA_FOLDER).absolutePath
            type = com.owncloud.gshare.datamodel.MediaFolderType.VIDEO
            numberOfFiles = 0L
        }
        Assert.assertFalse(SyncedFolderUtils.isQualifyingMediaFolder(folder))
    }
    ...
    fun assertMediaFolderImagesQualified() {
        val folder = com.owncloud.gshare.datamodel.MediaFolder().apply {
            absolutePath = getDummyFile(THUMBDATA_FOLDER).absolutePath
            type = com.owncloud.gshare.datamodel.MediaFolderType.IMAGE
            numberOfFiles = 4L
    ...
    fun assertSyncedFolderNullSafe() {
        val folder: com.owncloud.gshare.datamodel.SyncedFolder? = null
        Assert.assertFalse(SyncedFolderUtils.isQualifyingMediaFolder(folder))
    }
    ...
    fun assertUnqualifiedContentSyncedFolder() {
        ...
        val folder = com.owncloud.gshare.datamodel.SyncedFolder(
            ...
            true,
            0L,
            com.owncloud.gshare.datamodel.MediaFolderType.IMAGE,
            false
        )
        Assert.assertFalse(SyncedFolderUtils.isQualifyingMediaFolder(folder))
    }
    ...
    fun assertUnqualifiedSyncedFolder() {
        ...
        val folder = com.owncloud.gshare.datamodel.SyncedFolder(
            com.owncloud.gshare.utils.FileStorageUtils.getTemporalPath(account.name) + File.separatorChar + THUMBNAILS_FOLDER,
            "",
            true,
            false,
            false,
    ...
    companion object {
        ...
        fun setUp() {
            val tempPath =
                File(
                    com.owncloud.gshare.utils.FileStorageUtils.getTemporalPath(account.name) + File.separatorChar +
                        THUMBNAILS_FOLDER
                )
            if (!tempPath.exists()) {
                tempPath.mkdirs()
        ...
        @JvmStatic
        fun tearDown() {
            FileUtils.deleteDirectory(File(com.owncloud.gshare.utils.FileStorageUtils.getTemporalPath(account.name)))
        }
    }
}
