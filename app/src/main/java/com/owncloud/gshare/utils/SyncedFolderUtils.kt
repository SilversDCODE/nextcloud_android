...
object SyncedFolderUtils {
    ...
    private val AUTO_QUALIFYING_FOLDER_TYPE_SET: Set<_root_ide_package_.com.owncloud.gshare.datamodel.MediaFolderType> = setOf(
     ...
     */
    fun isQualifyingMediaFolder(mediaFolder: _root_ide_package_.com.owncloud.gshare.datamodel.MediaFolder?): Boolean {
        return when {
            ...
            !isQualifiedFolder(mediaFolder.absolutePath) -> false
                ...
                when {
                    ...
                    mediaFolder.type == _root_ide_package_.com.owncloud.gshare.datamodel.MediaFolderType.IMAGE -> containsQualifiedImages(
     ...
     * @return `true` if it qualifies as a media folder else `false`
     */
    fun isQualifyingMediaFolder(syncedFolder: _root_ide_package_.com.owncloud.gshare.datamodel.SyncedFolder?): Boolean {
     ...
     * @return `true` if it qualifies as a media folder else `false`
     */
    fun isQualifyingMediaFolder(folderPath: String?, folderType: _root_ide_package_.com.owncloud.gshare.datamodel.MediaFolderType): Boolean {
        return when {
            ...
            folderPath == null -> false
                ...
                when {
                    ...
                    // music album (just one cover-art image)
                    folderType == _root_ide_package_.com.owncloud.gshare.datamodel.MediaFolderType.IMAGE -> containsQualifiedImages(files)
                    else -> true
                }
            }
        }
    ...
    private fun containsQualifiedImages(files: List<File>): Boolean {
        return files.any { isFileNameQualifiedForAutoUpload(it.name) && _root_ide_package_.com.owncloud.gshare.utils.MimeTypeUtil.isImage(it) }
    }
