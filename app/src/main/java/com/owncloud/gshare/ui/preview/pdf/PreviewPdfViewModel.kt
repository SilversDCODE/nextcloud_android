...
class PreviewPdfViewModel @Inject constructor(val appPreferences: AppPreferences) : ViewModel() {
     ...
     */
    fun process(file: _root_ide_package_.com.owncloud.gshare.datamodel.OCFile) {
        closeRenderer()
        _pdfRenderer.value =
    ...
    fun onClickPage(page: Bitmap) {
        val file = File.createTempFile("pdf_page", ".bmp", _root_ide_package_.com.owncloud.gshare.MainApp.getAppContext().cacheDir)
