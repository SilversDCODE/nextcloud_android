...
class UnifiedSearchItemViewHolder(
    ...
    fun bind(entry: SearchResultEntry) {
        ...
        val mimetype = _root_ide_package_.com.owncloud.gshare.utils.MimeTypeUtil.getBestMimeTypeByFilename(entry.title)

        val placeholder = getPlaceholder(entry, mimetype)

        Glide.with(context).using(
            _root_ide_package_.com.owncloud.gshare.utils.glide.CustomGlideStreamLoader(
                user,
                clientFactory
            )
        )
        ...
        val drawable = with(entry.icon) {
            when {
                ...
                startsWith("icon-deck") ->
                    ...
                    _root_ide_package_.com.owncloud.gshare.utils.MimeTypeUtil.getFileTypeIcon(mimetype, entry.title, context, viewThemeUtils)
            }
        }
    ...
    private inner class RoundIfNeededListener(private val entry: SearchResultEntry) :
        ...
        override fun onResourceReady(
            ...
            if (entry.rounded) {
                val drawable = _root_ide_package_.com.owncloud.gshare.utils.BitmapUtils.bitmapToCircularBitmapDrawable(context.resources, resource)
