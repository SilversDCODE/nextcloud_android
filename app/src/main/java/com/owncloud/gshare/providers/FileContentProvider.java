...
public class FileContentProvider extends ContentProvider {
    ...
    private static final String[] PROJECTION_CONTENT_TYPE = new String[]{
        ProviderMeta.ProviderTableMeta._ID, ProviderMeta.ProviderTableMeta.FILE_CONTENT_TYPE
    };
    private static final String[] PROJECTION_REMOTE_ID = new String[]{
        ProviderMeta.ProviderTableMeta._ID, ProviderMeta.ProviderTableMeta.FILE_REMOTE_ID
    };
    private static final String[] PROJECTION_FILE_PATH_AND_OWNER = new String[]{
        ProviderMeta.ProviderTableMeta._ID, ProviderMeta.ProviderTableMeta.FILE_PATH, ProviderMeta.ProviderTableMeta.FILE_ACCOUNT_OWNER
    };
        ...
        if (children != null) {
            if (children.moveToFirst()) {
                ...
                while (!children.isAfterLast()) {
                    childId = children.getLong(children.getColumnIndexOrThrow(ProviderMeta.ProviderTableMeta._ID));
                    isDir = MimeType.DIRECTORY.equals(children.getString(
                        children.getColumnIndexOrThrow(ProviderMeta.ProviderTableMeta.FILE_CONTENT_TYPE)
                                                                        ));
                    if (isDir) {
                        count += delete(db, ContentUris.withAppendedId(ProviderMeta.ProviderTableMeta.CONTENT_URI_DIR, childId),
                                        null, (String[]) null);
                    } else {
                        count += delete(db, ContentUris.withAppendedId(ProviderMeta.ProviderTableMeta.CONTENT_URI_FILE, childId),
                                        null, (String[]) null);
                    }
                    children.moveToNext();
                }
    ...
    private int deleteSingleFile(SupportSQLiteDatabase db, Uri uri, String where, String... whereArgs) {
        ...
        try (Cursor c = query(db, uri, PROJECTION_REMOTE_ID, where, whereArgs, null)) {
            if (c.moveToFirst()) {
                String id = c.getString(c.getColumnIndexOrThrow(ProviderMeta.ProviderTableMeta._ID));
    ...
    private int deleteWithUri(SupportSQLiteDatabase db, Uri uri, String where, String[] whereArgs) {
        ...
        return db.delete(ProviderMeta.ProviderTableMeta.FILE_TABLE_NAME,
    ...
    private Uri insert(SupportSQLiteDatabase db, Uri uri, ContentValues values) {
        ...
        switch (mUriMatcher.match(uri)) {
            case ROOT_DIRECTORY:
            case SINGLE_FILE:
                String where = ProviderMeta.ProviderTableMeta.FILE_PATH + "=? AND " + ProviderMeta.ProviderTableMeta.FILE_ACCOUNT_OWNER + "=?";
                ...
                if (!doubleCheck.moveToFirst()) {
                    doubleCheck.close();
                    long rowId = db.insert(ProviderMeta.ProviderTableMeta.FILE_TABLE_NAME, SQLiteDatabase.CONFLICT_REPLACE, values);
                    if (rowId > 0) {
                    ...
                    Uri insertedFileUri = ContentUris.withAppendedId(
                        ...
                        doubleCheck.getLong(doubleCheck.getColumnIndexOrThrow(ProviderMeta.ProviderTableMeta._ID))
                    );
                    doubleCheck.close();
            ...
            case SHARES:
                ...
                long idShares = db.insert(ProviderMeta.ProviderTableMeta.OCSHARES_TABLE_NAME, SQLiteDatabase.CONFLICT_REPLACE, values);
                if (idShares > 0) {
                    insertedShareUri = ContentUris.withAppendedId(ProviderMeta.ProviderTableMeta.CONTENT_URI_SHARE, idShares);
                } else {
                    throw new SQLException(ERROR + uri);
            ...
            case CAPABILITIES:
                Uri insertedCapUri;
                long idCapabilities = db.insert(ProviderMeta.ProviderTableMeta.CAPABILITIES_TABLE_NAME, SQLiteDatabase.CONFLICT_REPLACE, values);
                if (idCapabilities > 0) {
                    insertedCapUri = ContentUris.withAppendedId(ProviderMeta.ProviderTableMeta.CONTENT_URI_CAPABILITIES, idCapabilities);
                } else {
                    throw new SQLException(ERROR + uri);
                }
            ...
            case UPLOADS:
                ...
                long uploadId = db.insert(ProviderMeta.ProviderTableMeta.UPLOADS_TABLE_NAME, SQLiteDatabase.CONFLICT_REPLACE, values);
                if (uploadId > 0) {
                    insertedUploadUri = ContentUris.withAppendedId(ProviderMeta.ProviderTableMeta.CONTENT_URI_UPLOADS, uploadId);
                } else {
                    throw new SQLException(ERROR + uri);
                }
            ...
            case SYNCED_FOLDERS:
                ...
                long syncedFolderId = db.insert(ProviderMeta.ProviderTableMeta.SYNCED_FOLDERS_TABLE_NAME, SQLiteDatabase.CONFLICT_REPLACE, values);
                if (syncedFolderId > 0) {
                    insertedSyncedFolderUri = ContentUris.withAppendedId(ProviderMeta.ProviderTableMeta.CONTENT_URI_SYNCED_FOLDERS,
                                                                         syncedFolderId);
                } else {
                    throw new SQLException("ERROR " + uri);
                }
            ...
            case EXTERNAL_LINKS:
                ...
                long externalLinkId = db.insert(ProviderMeta.ProviderTableMeta.EXTERNAL_LINKS_TABLE_NAME, SQLiteDatabase.CONFLICT_REPLACE, values);
                if (externalLinkId > 0) {
                    insertedExternalLinkUri = ContentUris.withAppendedId(ProviderMeta.ProviderTableMeta.CONTENT_URI_EXTERNAL_LINKS,
                                                                         externalLinkId);
                } else {
                    throw new SQLException("ERROR " + uri);
                }
            ...
            case VIRTUAL:
                ...
                long virtualId = db.insert(ProviderMeta.ProviderTableMeta.VIRTUAL_TABLE_NAME, SQLiteDatabase.CONFLICT_REPLACE, values);
            ...
            case FILESYSTEM:
                ...
                long filesystemId = db.insert(ProviderMeta.ProviderTableMeta.FILESYSTEM_TABLE_NAME, SQLiteDatabase.CONFLICT_REPLACE, values);
                if (filesystemId > 0) {
                    insertedFilesystemUri = ContentUris.withAppendedId(ProviderMeta.ProviderTableMeta.CONTENT_URI_FILESYSTEM,
                                                                       filesystemId);
                } else {
                    throw new SQLException("ERROR " + uri);
                }
    ...
    private void updateFilesTableAccordingToShareInsertion(SupportSQLiteDatabase db, ContentValues newShare) {
        ...
        ShareType newShareType = ShareType.fromValue(newShare.getAsInteger(ProviderMeta.ProviderTableMeta.OCSHARES_SHARE_TYPE));
        ...
        db.update(ProviderMeta.ProviderTableMeta.FILE_TABLE_NAME, SQLiteDatabase.CONFLICT_REPLACE, fileValues, where, whereArgs);
    }
    ...
    private Cursor query(SupportSQLiteDatabase db,
        ...
        switch (uriMatch) {
            ...
            case SINGLE_FILE:
                ...
                tableName = ProviderMeta.ProviderTableMeta.FILE_TABLE_NAME;
                break;
            case SHARES:
                tableName = ProviderMeta.ProviderTableMeta.OCSHARES_TABLE_NAME;
                break;
            case CAPABILITIES:
                tableName = ProviderMeta.ProviderTableMeta.CAPABILITIES_TABLE_NAME;
                break;
            case UPLOADS:
                tableName = ProviderMeta.ProviderTableMeta.UPLOADS_TABLE_NAME;
                break;
            case SYNCED_FOLDERS:
                tableName = ProviderMeta.ProviderTableMeta.SYNCED_FOLDERS_TABLE_NAME;
                break;
            case EXTERNAL_LINKS:
                tableName = ProviderMeta.ProviderTableMeta.EXTERNAL_LINKS_TABLE_NAME;
                break;
            case VIRTUAL:
                tableName = ProviderMeta.ProviderTableMeta.VIRTUAL_TABLE_NAME;
                break;
            case FILESYSTEM:
                tableName = ProviderMeta.ProviderTableMeta.FILESYSTEM_TABLE_NAME;
                break;
            default:
                throw new IllegalArgumentException("Unknown uri id: " + uri);
        }
        ...
        if (uriMatch != ROOT_DIRECTORY && uri.getPathSegments().size() > SINGLE_PATH_SEGMENT) {
            String idColumn = uriMatch == DIRECTORY ? ProviderMeta.ProviderTableMeta.FILE_PARENT : ProviderMeta.ProviderTableMeta._ID;
    ...
    private int update(SupportSQLiteDatabase db, Uri uri, ContentValues values, String selection, String... selectionArgs) {
        ...
        switch (mUriMatcher.match(uri)) {
            case DIRECTORY:
                return 0;
            case SHARES:
                return db.update(ProviderMeta.ProviderTableMeta.OCSHARES_TABLE_NAME, SQLiteDatabase.CONFLICT_REPLACE, values, selection, selectionArgs);
            case CAPABILITIES:
                return db.update(ProviderMeta.ProviderTableMeta.CAPABILITIES_TABLE_NAME, SQLiteDatabase.CONFLICT_REPLACE, values, selection, selectionArgs);
            case UPLOADS:
                return db.update(ProviderMeta.ProviderTableMeta.UPLOADS_TABLE_NAME, SQLiteDatabase.CONFLICT_REPLACE, values, selection, selectionArgs);
            case SYNCED_FOLDERS:
                return db.update(ProviderMeta.ProviderTableMeta.SYNCED_FOLDERS_TABLE_NAME, SQLiteDatabase.CONFLICT_REPLACE, values, selection, selectionArgs);
            case FILESYSTEM:
                return db.update(ProviderMeta.ProviderTableMeta.FILESYSTEM_TABLE_NAME, SQLiteDatabase.CONFLICT_REPLACE, values, selection, selectionArgs);
            default:
                return db.update(ProviderMeta.ProviderTableMeta.FILE_TABLE_NAME, SQLiteDatabase.CONFLICT_REPLACE, values, selection, selectionArgs);
        }
    }
    ...
    static class VerificationUtils {
        ...
        private static boolean isValidColumnName(@NonNull String columnName) {
            return ProviderMeta.ProviderTableMeta.FILE_ALL_COLUMNS.contains(columnName);
        }
